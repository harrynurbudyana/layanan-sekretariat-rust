...
{#if admin.isModalOpen}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in duration-200">
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative">
      <button 
      ...
      <div class="flex items-center gap-3 mb-4">
        <div class="w-10 h-10 rounded-xl bg-red-100 dark:bg-red-950/60 text-red-600 dark:text-red-400 flex items-center justify-center shadow-md shadow-red-600/10 ring-1 ring-red-500/20">
      ...
      <div class="space-y-3">
        ...
        <div class="flex gap-2 pt-2">
          ...
          >
            {admin.loading ? 'Memverifikasi...' : 'Buka Akses'}
          </button>
        </div>
      </div>
    </div>
